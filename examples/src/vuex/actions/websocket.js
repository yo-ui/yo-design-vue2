...
export default {
  ...
  startWebsocket(context, options) {
    fqCommon.log(
      "当前打开的数据推送通道为",
      options,
      $vm.websocket && $vm.websocket.readyState
    );
    ...
    if (
      !$vm.websocket ||
      $vm.websocket.readyState == $vm.websocket.CONNECTING
    ) {
      setTimeout(() => {
          ...
          if (result && typeof result === "object") {
            ...
            switch (event) {
              ...
              case Constants.WEBSOCKETEVENT.TRADE: //最新交易
                //交易中心最新成交数据
                let fq_exchange_last_trade_component = document.querySelector(
                  ".fq-exchange-last-trade-component"
                );
              ...
              case Constants.WEBSOCKETEVENT.DEPTHS: //深度数据
                //交易中心深度数据
                let fq_exchange_page_obj = document.querySelector(
                  ".fq-exchange-page"
                );
              ...
              case Constants.WEBSOCKETEVENT.KLINEMESSAGE: //k线数据
                //首页K线数据
                let fq_index_kline_component = document.querySelector(
                  ".fq-index-kline-component"
                );
                ...
                //交易中心K线数据
                let fq_exchange_kline_component = document.querySelector(
                  ".fq-exchange-kline-component"
                );
                ...
                //交易中心K线数据
                let fq_exchange_trade_kline_component = document.querySelector(
                  ".fq-exchange-trade-kline-component"
                );
              ...
              case Constants.WEBSOCKETEVENT.ORDERUNDOSUCCESS: //订单撤销成功的推送
                ...
                //撤单成功刷新交易组件
                let $ref_exchange_com = document.querySelector(
                  ".fq-exchange-page"
                );
                if ($ref_exchange_com) {
              ...
              case Constants.WEBSOCKETEVENT.ORDERPFL: //下单成功
                ...
                if (authorizeList) {
                  authorizeList.forEach(item => {
                    if (item.code == result.code) {
                      //如果推送的订单号一样
                      if (
                        Number(item.orderTypeName) === 0 &&
                        Number(item.directionName) === 0
                      ) {
                        // 当前订单为市价且交易类型为买时
                ...
                if (historyAuthorizeList) {
                  ...
                  context.commit(
                    "setHistoryAuthorizeList",
                    historyAuthorizeList
                  );
                }
                ...
                //下单处理刷新交易组件
                let $ref_exchange_com_obj = document.querySelector(
                  ".fq-exchange-page"
                );
                if ($ref_exchange_com_obj) {
                  let $trade_vm =
                    $ref_exchange_com_obj.__vue__.$refs.fq_trade_com;
                  $trade_vm && $trade_vm.refreshTrade();
                }
                break;
